...
describe('temba-chart', () => {
    ...
    it('configures scales correctly for horizontal charts', async () => {
        ...
        expect(chart.chart.options.scales.x.type).to.equal('category');
        expect(chart.chart.options.scales.y.min).to.equal(0);
        expect(chart.chart.options.scales.y.stacked).to.equal(true);
        // Test horizontal chart
        chart.horizontal = true;
        await chart.updateComplete;
        await new Promise((resolve) => setTimeout(resolve, 100));
        // In horizontal charts: x-axis should be for values, y-axis for categories
        expect(chart.chart.options.scales.x.min).to.equal(0);
        expect(chart.chart.options.scales.x.stacked).to.equal(true);
        expect(chart.chart.options.scales.y.type).to.equal('category');
    });
});
