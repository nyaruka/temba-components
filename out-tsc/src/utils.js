...
export var Color;
(function (Color) {
    Color["BLUE"] = "color:#5078b5;";
    Color["GREEN"] = "color:#62bd6a;";
    Color["RED"] = "color:#e36049;";
    Color["PURPLE"] = "color:#a626a4;";
})(Color || (Color = {}));
export const log = (message, styling = '', details = []) => {
    if (styling === '') {
...
export const fetchResultsPage = (url, controller = null, headers = {}) => {
    return new Promise((resolve, reject) => {
...
export const fetchResults = async (url, headers = {}) => {
    if (!url) {
...
export const postForm = (url, payload, headers = {}) => {
...
export const capitalize = ([first, ...rest], locale = navigator.language) => first === undefined ? '' : first.toLocaleUpperCase(locale) + rest.join('');
...
export const formatFileSize = (bytes, decimalPoint) => {
    if (bytes == 0)
        return '0 KB';
    const k = 1024, dm = decimalPoint || 2, sizes = ['B', 'KB', 'MB', 'GB'], //, 'TB', 'PB', 'EB', 'ZB', 'YB'],
...
export const extractInitials = (text) => {
    ...
    let words = text.match(/(([\p{L}\p{N}]+-[\p{L}\p{N}]+)|([\p{L}\p{N}]+))/gu) || [];
    if (words.length == 1) {
...
export const showModax = (title, endpoint) => {
    window.showModax(title, endpoint);
};
export const sanitizeUnintendedUnicode = (text) => {
    if (text) {
        return text
