 ...
 */
export const getCompletions = (schema, dotQuery, session, store) => {
...
export const executeCompletionQuery = (ele, session) => {
    const result = {
    ...
    if (currentExpression) {
        ...
        for (let i = currentExpression.text.length; i >= 0; i--) {
            ...
            if (curr === '@' ||
                ...
                result.options = [
                    ...getCompletions(store.getCompletionSchema(), result.query, session, store),
                    ...(includeFunctions
