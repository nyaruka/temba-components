...
export class ContactSearch extends FormElement {
    ...
    handleActivityLevelChanged(evt) {
    ...
    handleActivityLabelClicked(evt) {
        if (evt.target &&
    ...
    handleExclusionChanged(evt) {
        if (evt.target.tagName === 'TEMBA-CHECKBOX') {
            ...
            if (checkbox.name === 'not_seen_since_days' && value) {
                const select = checkbox.parentElement.querySelector('temba-select');
                if (select.values[0]) {
              ...
              ${this.not_seen_since_days ||
                ...
                ? html `
                    ...
                    ? html `<temba-checkbox
                            ...
                            label="${msg('Have not started this flow in the last 90 days')}"
