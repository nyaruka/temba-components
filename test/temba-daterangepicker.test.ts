...
import { assertScreenshot, getAttributes, getClip, getComponent } from './utils.test';

export const getRangePickerHTML = (attrs: any = {}) => {
  return `<temba-daterangepicker ${getAttributes(attrs)}></temba-daterangepicker>`;
};
...
describe('temba-daterangepicker', () => {
  it('can create a date range picker', async () => {
    const picker: DateRangePicker = await createRangePicker(getRangePickerHTML());
  ...
  it('opens dropdown when display is clicked', async () => {
    ...
    const rangeDisplay = picker.shadowRoot?.querySelector('.range-display') as HTMLElement;
    rangeDisplay.click();
  ...
  it('validates that end date is after start date', async () => {
    ...
    const startInput = picker.shadowRoot?.querySelector('.date-inputs input[type="date"]') as HTMLInputElement;
    const endInput = picker.shadowRoot?.querySelectorAll('.date-inputs input[type="date"]')[1] as HTMLInputElement;
    ...
    const applyButton = picker.shadowRoot?.querySelector('.actions button.primary') as HTMLElement;
    applyButton.click();
    await picker.updateComplete;
    ...
    expect(errorMessage?.textContent).to.contain('End date must be after start date');
  });
  ...
  it('cancels changes and closes dropdown', async () => {
    ...
    // Cancel changes
    const cancelButton = picker.shadowRoot?.querySelector('.actions button:not(.primary)') as HTMLElement;
    cancelButton.click();
  ...
  it('serializes value as JSON string', async () => {
    ...
    expect(serialized).to.equal('{"startDate":"2024-01-01","endDate":"2024-01-15"}');
  });
