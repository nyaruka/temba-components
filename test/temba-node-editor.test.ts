...
describe('temba-node-editor', () => {
  ...
  it('flushes rich-edit values before saving rules', async () => {
    const node = {
      uuid: 'test-node-uuid',
      actions: [],
      router: {
    ...
    expect(rulesEditor).to.not.be.null;

    const valueEditor = rulesEditor.shadowRoot?.querySelector(
      'temba-rich-edit[name="value1"]'
    ) as any;
    expect(valueEditor).to.not.be.null;

    // Simulate a value change that hasn't propagated via input/change yet.
    valueEditor.value = '@contact.name';
