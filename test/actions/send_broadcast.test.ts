...
describe('send_broadcast action config', () => {
  ...
  describe('action validation', () => {
    it('validates that recipients are required', () => {
      ...
      const result = send_broadcast.validate(action);
    ...
    it('validates that message text is required', () => {
      const action: SendBroadcast = {
        uuid: 'test-uuid',
        type: 'send_broadcast',
        text: '',
    ...
    it('validates that whitespace-only text is not allowed', () => {
      const action: SendBroadcast = {
        uuid: 'test-uuid',
        type: 'send_broadcast',
        text: '   \n\t  ',
    ...
    it('validates that both recipients and text are required', () => {
      const action: SendBroadcast = {
        uuid: 'test-uuid',
        type: 'send_broadcast',
        text: '',
    ...
    it('validates attachment count limit', () => {
      const action: SendBroadcast = {
        ...
        groups: [{ uuid: 'group-1', name: 'Test Group' }],
        contacts: [],
        attachments: new Array(11).fill('image/jpeg:test.jpg')
      };

      const result = send_broadcast.validate(action);
      expect(result.valid).to.be.false;
      expect(result.errors.text).to.equal(
        'Each broadcast can only have up to 10 attachments'
      );
    });

    it('passes validation with valid recipients and text', () => {
      const action: SendBroadcast = {
        uuid: 'test-uuid',
        type: 'send_broadcast',
        text: 'Valid message',
    ...
    it('passes validation with groups', () => {
      const action: SendBroadcast = {
        uuid: 'test-uuid',
        type: 'send_broadcast',
        text: 'Valid message',
