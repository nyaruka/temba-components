...
function bundleNotFoundError(name) {
    return (0, utils_js_1.createError)(`Could not find any @web/rollup-plugin-html output named ${name}.`);
}
function createEntrypoints(bundle, filetype) {
    if (!bundle.options.format) {
        throw (0, utils_js_1.createError)('An output format must be configured');
    }
...
function createPolyfillsLoaderConfig(pluginOptions, bundle, bundles) {
    ...
    if (bundle) {
        if (modernOutput && legacyOutput) {
            throw (0, utils_js_1.createError)('Options modernOutput and legacyOutput was set, but @web/rollup-plugin-html' +
        ...
        if (!bundles || Object.keys(bundles).length === 0) {
            throw (0, utils_js_1.createError)('@web/rollup-plugin-html did not output any bundles to be injected');
        }
        if (!modernOutput || !legacyOutput) {
            throw (0, utils_js_1.createError)('Rollup is configured to output multiple builds, set the modernOutput and legacyOutput options' +
