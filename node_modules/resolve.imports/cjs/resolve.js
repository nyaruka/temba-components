...
const pattern_key_compare_1 = require("pattern-key-compare");
/**
 ...
 */
function resolve(pjson, specifier, options) {
    if (!pjson.imports)
        return undefined;
...
function getExpensionKeys(keys) {
    return keys.filter(k => k.indexOf('*') >= 0).sort(pattern_key_compare_1.patternKeyCompare);
}
